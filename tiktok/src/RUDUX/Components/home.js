...
// import {actionAdd, activeId} from '../action/actionAdd'
// import { addList, activeSplice } from '../reducer'
...
const HomePage = () => {
    ...
    const idactive = useSelector((state) => state.activeId)
    console.log(idactive);
    localStorage.setItem('id', JSON.stringify(idactive.Id))
    const dispach = useDispatch()
    const handleAdd = () => {
        dispach(addList.actions.addTodoList(job))
        dispach(activeSplice.actions.changeRandom(job))
        setJob('')
    }
    const handeActive = (id) => {
        dispach(activeSplice.actions.changeId(id))
    }
    return <>
        <div>
            <h1>XIN CHÀO CẢ NHÀ</h1>
        ...
        <ul>
            {jobs.map((job, id) => {
                return <li className={clsx('test3', (idactive.Id === id ? 'active' : undefined))} key={id} onClick={() => handeActive(id)}>{job}</li>
            })}
        </ul>
        <div className='container-grid'>
            <div className='item item-1'>header</div>
            <div className='item item-2'>slibar</div>
            <div className='item item-3'>conten1</div>
            <div className='item item-4'>conten2</div>
